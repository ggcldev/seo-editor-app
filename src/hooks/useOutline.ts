...
export function extractHeadings(md: string): Heading[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (atx) {
      ...
      if (text) items.push({ level: atx[1].length as 1 | 2 | 3, text, id: toId(text) });
      continue;
    }
    ...
    if (/^=+\s*$/.test(next) && text) {
      items.push({ level: 1, text, id: toId(text) });
      i++;
    } else if (/^-+\s*$/.test(next) && text) {
      items.push({ level: 2, text, id: toId(text) });
      i++;
    }
  }
  return items;
